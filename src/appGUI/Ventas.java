...
public class Ventas extends javax.swing.JPanel {
        ...
        if(!(cb||sp)){
            try{
               ...
               String[] lineas=new String[1000];
                ...
                if (!(cod.equals(codigo))){lineas[n]=linea;}
                else {c++;
                        if(c==4){int ca=(Integer.parseInt(linea))-s;lineas[n]=""+ca;}
                        else lineas[n]=linea;}
                }registro.close();
                ...
                escritor.close();
            }catch(IOException e){System.out.println("Excepcion en AgregarBoton");}
             LeerDatos();
        }
    }
    ...
    public void LeerDatos(){
		...
		n=0; String cod=null;
                ...
                while ((linea= registro.readLine())!=null){n++;System.out.println(linea);
		if ((n-1)%6==0){cbCodigo.addItem(cod=linea);}
                if ((n-4)%6==0) if((cant=(Integer.parseInt(linea)))==0)cbCodigo.removeItem(cod);
            }registro.close();
	...
	private void cbCodigoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbCodigoActionPerformed
        ...
        if(!codigo.equals("Seleccionar")){
            String cod=null;
            try{registro=new DataInputStream(new FileInputStream("Refaccionaria.txt"));
            n=0; int c=0;
            while ((linea= registro.readLine())!=null){n++;System.out.println(linea);
            if ((n-1)%6==0){cod=linea;}
            if (cod.equals(codigo)){c++;
            if(c==2)tfArticulo.setText(linea);
            if(c==3)cbTipo.setSelectedItem(linea);
            if(c==4){cant=Integer.parseInt(linea);}
            if(c==5)ffPrecio.setText(linea);}
            }registro.close();
